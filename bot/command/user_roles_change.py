...
class AddUserRole(command.Command):
    ...
    async def handle(self):
        ...
        if found_student:
            member = await self.message.guild.fetch_member(found_student[self.__DISCORD_ID])
            await ra.add_role(member, role_name)
            await self.message.channel.send('Updated user %s with role %s' % (user_name, role_name))
        elif found_ta:
            member = await self.message.guild.fetch_member(found_ta[self.__DISCORD_ID])
            await ra.add_role(member, role_name)
            await self.message.channel.send('Updated user %s with role %s' % (user_name, role_name))
        elif found_admin:
            member = await self.message.guild.fetch_member(found_admin[self.__DISCORD_ID])
            await ra.add_role(member, role_name)
            await self.message.channel.send('Updated user %s with role %s' % (user_name, role_name))
        else:
