...
class StartAttendance(command.Command):
    ...
    __COMMAND_REGEX = r'!attendance\s+start\s+lecture\s*=\s*(?P<lecture_name>[\w_-]+)\s+(time\s*=\s*(?P<timer>\d{2}:\d{2}:\d{2}))?'
    ...
    async def handle(self, new_message=None):
        ...
        for session_key in lecture_attendance_sessions[self.__SESSIONS]:
            session_start_time = datetime.strptime(lecture_attendance_sessions[self.__SESSIONS][session_key][self.__START_TIME], "%Y%M%D:%X")
